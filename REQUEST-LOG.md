...
| 28 | Clarified: avatar should be a real animated, voice-driven, talking character with no style presets, driven by free-text prompt, not an image generator — called out #27 as a failure to follow instructions | Acknowledged the miss directly; researched real real-time avatar providers (Tavus, LemonSlice, HeyGen, D-ID, Synthesia) with pricing/capability comparison; asked which provider to integrate (requires a new paid API key the agent cannot create itself) | ⏳ **Pending** — awaiting provider choice/API key before building |
